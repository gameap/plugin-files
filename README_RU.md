...
`make test` прогоняет роутер и слой обработчиков нативно против моканного
хоста в памяти плюс vitest фронтенда. Ни то, ни другое панель не поднимает.

Сквозной набор в [`e2e/`](e2e/README.md) поднимает: он загружает `files.wasm` в
настоящую панель, ставит gameap-files на настоящую ноду с админской страницы,
создаёт FTP-пользователя на игровом сервере и входит этим пользователем по FTP
и SFTP. Гоняется на Linux и на Windows, потому что установщики, менеджеры
сервисов и пути там разные.

| Команда                          | Что делает                                       |
|----------------------------------|--------------------------------------------------|
| `make e2e-deps`                  | Ставит набор и его сборку Chromium                |
| `make e2e`                       | Гоняет его против уже поднятых панели и ноды      |
| `e2e/scripts/provision-linux.sh` | Поднимает эти панель и ноду на Linux-хосте        |

В CI это workflow `E2E`: ночью с гейтом по коммитам, по требованию через
`workflow_dispatch` и на pull request только если на нём висит метка `e2e`.
Матрица покрывает последний релиз панели и сборку панели из `main`.
